 ...
 * @version 1
 */
public class ImportParser extends XMLDOMElementParser
{
    
    public ImportParser(BSDLRegistry bsdlRegistry)
    {
        super(XMLSyntax.IMPORT(), bsdlRegistry, Node.ELEMENT_NODE);
    }

    @Override
    protected ISyntaxElement parse(DOMNodeContainer node) throws ParsingException
    {
        return (Import)(this.parsedElement = new Import());
    }

    @Override
    public IElementParser newInstance()
    {
        return new ImportParser(bsdlRegistry);
    }

    @Override
    public boolean assignParsedElement(String syntaxElementName, ISyntaxElement syntaxElement) throws ModelException
    {
        return this.axiomAssign.assignParsedElement((Import)this.parsedElement, syntaxElementName, syntaxElement);
    }
