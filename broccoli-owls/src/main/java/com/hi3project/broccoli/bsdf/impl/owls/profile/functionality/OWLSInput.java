/*******************************************************************************
 *
 * Copyright (C) 2015 Mytech Ingenieria Aplicada <http://www.mytechia.com>
 * Copyright (C) 2015 Alejandro Paz <alejandropl@lagostelle.com>
 *
 * This file is part of Broccoli.
 *
 * Broccoli is free software: you can redistribute it and/or modify it under the
 * terms of the GNU Affero General Public License as published by the Free
 * Software Foundation, either version 3 of the License, or (at your option) any
 * later version.
 *
 * Broccoli is distributed in the hope that it will be useful, but WITHOUT ANY
 * WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR
 * A PARTICULAR PURPOSE. See the GNU Affero General Public License for more
 * details.
 *
 * You should have received a copy of the GNU Affero General Public License
 * along with Broccoli. If not, see <http://www.gnu.org/licenses/>.
 *
 ******************************************************************************/

package com.hi3project.broccoli.bsdf.impl.owls.profile.functionality;

import com.hi3project.broccoli.bsdl.impl.exceptions.ModelException;
import com.hi3project.broccoli.bsdl.impl.SemanticIdentifier;
import com.hi3project.broccoli.bsdm.api.profile.functionality.IInput;
import com.hi3project.broccoli.bsdl.api.ISemanticIdentifier;
import org.mindswap.owls.process.variable.Input;

/**
 *
 * 
 */
public class OWLSInput implements IInput {
    
    private String name = null;
    private SemanticIdentifier semanticAnnotation = null;
      
    
    public OWLSInput(Input input) throws ModelException {
        name = input.getName();
        if (input.getParamType().isDataType()) {
            semanticAnnotation = new SemanticIdentifier(input.getParamType().getURI().toString());
        } else {
            semanticAnnotation = new SemanticIdentifier(input.getParamType().toString());
        }        
    }

    @Override
    public String name() {
        return name;
    }

    @Override
    public ISemanticIdentifier getSemanticAnnotation() {
        return semanticAnnotation;
    }
    
}
